...
public record JsonPlanarCube(Map<Face, JsonFaceSet> faces) implements ModelBoxComponent {
    ...
    public JsonPlanarCube(FileContent<JsonElement> context, JsonObject json) {
        this(new EnumMap<>(Face.class));
        Face.VALUES.forEach(face -> {
            JsonUtil.accept(json, face.name().toLowerCase())
                .map(JsonElement::getAsJsonArray)
                .map(el -> new JsonFaceSet(context, el, face))
                .filter(f -> !f.elements.isEmpty())
                .ifPresent(el -> faces.put(face, el));
        });
    }

    @Override
    public BoxBuilder builder(ModelContext context) {
        return BoxBuilder.union(context, faces.values().stream().flatMap(face -> face.exportMulti(context)), ID);
    }
    ...
    public static record JsonFaceSet(Face face, List<JsonFace> elements) {
        ...
        public BoxBuilder exportSingle(ModelContext context) {
            return BoxBuilder.union(context, exportMulti(context), ID);
        }

        public Stream<Pair<Face, BoxBuilder>> exportMulti(ModelContext subContext) {
            Fixtures fixtures = new Fixtures(subContext);
            return elements.stream().map(f -> Pair.of(face, f.builder(subContext, fixtures)));
        }
